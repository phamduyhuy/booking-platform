...
public class CoreAgent {
  ...
  private static final String SYSTEM_PROMPT = """
      ...
      - For brave search mcp always use country=US
      ...
      - **Images**: Use `brave_image_search` (country="US") to show visuals of destinations or hotels.

      ### 2. MAPBOX (Location & Places)
      - **Search**: Use `mapbox_search_places` (or available search tool) to find specific locations or **categories** (e.g., "restaurants", "cafes", "museums", "parks").
      - **Context**: When users ask for "places to eat", "attractions", or "what to do" in a city, use this tool.
      - **Parameters**: Provide a descriptive `q` (query).

      ### 3. OPENWEATHER (Weather)
      - **Current**: Use weather tools to check current conditions (requires lat/lon or city name).
      - **Forecast**: Use forecast tools for upcoming trips.
      - **Proactive**: Check weather for the destination when planning a trip. Warn about severe conditions.
  ...
  private final BeanOutputConverter<StructuredChatPayload> beanOutputConverter = new BeanOutputConverter<>(
      StructuredChatPayload.class);
  ...
  public Mono<StructuredChatPayload> processSyncStructured(String message, String conversationId, String userId) {
    ...
    return Mono.fromCallable(() -> {
      ...
      CallResponseSpec rawResponse = chatClient.prompt()
      ...
      if (rawResponse == null || rawResponse.content().isBlank()) {
      ...
      // Firt try to use entity method to parse response
      try {
  ...
  /**
   * Extract username from conversationId formatted as
   * {username}:{actualConversationId}
   * Note: conversationId contains username (e.g., "john.doe:conv-uuid"), not
   * userId (UUID)
   * 
